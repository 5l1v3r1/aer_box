...
!
  SUBROUTINE Do_Sect_Aer( am_I_Root, Input_Opt,  State_Met,  &
                          State_Chm, State_Diag, Run_Micro,  &
                          RC                                 )
!
! !USES:
!
...
!
    ...
    If (Run_Micro.and.LAER_sed) Then
       ...
       !$OMP PRIVATE(  T_Col,  P_Col, air_dens_Col, dz_col,  Sul_Col,  aero_vt_d ) &
       !$OMP SCHEDULE( DYNAMIC, 1                                                )
       Do I=1,IIPAR
       Do J=1,JJPAR
          ...
          Call AER_sedimentation(Sul_Col, T_Col, P_Col, aWP_Col, aDen_Col,   & 
                                 dt_step, air_dens_col, dz_col,  aero_vt_d)
...
! !INTERFACE:
!
  SUBROUTINE Init_Sect_Aer( am_I_Root, Input_Opt,  State_Chm, RC )
!
! !USES:
!
    USE CMN_SIZE_MOD,         Only : IIPAR, JJPAR, LLPAR
...
!    LOGICAL                :: prtDebug,  IsLocNoon
    ...
    Call AER_allocate_ini(am_I_Root,Input_Opt,State_Chm,RC)
    ...
    Allocate(aero_grow_d(IIPAR,JJPAR,LLPAR),STAT=AS)
    If (AS.ne.0) Then
    ...
    Allocate(aero_nrate_d(IIPAR,JJPAR,LLPAR),STAT=AS)
    If (AS.ne.0) Then
...
! !INPUT VARIABLES:
!           
    real(dp), intent(in) :: T_K(LLPAR)                  ! Temperature (K)
    real(dp), intent(in) :: P_hPa(LLPAR)                ! Pressure (hPa)
    !integer,  intent(in) :: LTROP
    real(dp), intent(in) :: dt                          ! Timestep length (s)
    real(dp), intent(in) :: dens(LLPAR)                 ! Air molecular number density (#/cm3)
    real(dp), intent(in) :: dz(LLPAR)                   ! Layer thickness (m)
    real(dp), intent(in) :: aWP_Col(LLPAR,n_aer_bin)    ! H2SO4 wt pcg (%)
    real(dp), intent(in) :: aDen_Col(LLPAR,n_aer_bin)   ! Aerosol density (g/cm3)
!    real(dp), intent(in) :: dt, xtm1(nbdim,nlev,n_aer_bin), pqm1(nbdim,nlev), pqte(nbdim,nlev)
!
! !OUTPUT VARIABLES:
!           
    real(dp), intent(inout)  :: Sul_vv(LLPAR,n_aer_bin)
    real(dp), intent(out)    :: aero_vt_d(LLPAR)
! 
! !REVISION HISTORY:
...
! !LOCAL VARIABLES:
!
    real(dp) :: vtte(LLPAR,n_aer_bin)
    integer  :: LLPAR1, jl, jk
    
    integer  :: ii,jj !counters !eth_af_dryS
    
    real(dp) ::  tc(LLPAR), vis(LLPAR), &
                 pq(LLPAR), fpair(LLPAR), dz2(LLPAR), &
                 vt(LLPAR,n_aer_bin), &
                 yt(LLPAR,n_aer_bin),yt1(LLPAR,n_aer_bin)
    logical :: ismaxmin(LLPAR)
    real(dp) :: v12, c12, a1, a2 
    
    real(dp) :: aer_vol_wet(LLPAR), aer_r_wet(LLPAR) !eth_af_dryS
    real(dp) :: aer_ar_wet(LLPAR) !eth_af_dryS
